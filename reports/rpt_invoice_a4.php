...
if(isset($_REQUEST['view_invoice_id'])) {
    ...
    if(!empty($invoice_id)) {
        ...
        if(!empty($invoice_list)) {
            foreach($invoice_list as $pi) {
                if(!empty($pi['invoice_number'])) {
    ...
    $pdf->SetX(105);
    $pdf->Cell(64,4,'Invoice No.            : '.$invoice_number,0,1,'L',0);
    $pdf->Cell(0,2,'',0,1,'L',0);
    $pdf->SetX(105);
    $pdf->Cell(64,4,'Date                        : '.$invoice_date,0,1,'L',0);
    $pdf->Cell(0,2,'',0,1,'L',0); 
    $pdf->SetX(105);
    ...
    $index = 0;
    $total_unit = $total_subunit = $invoice_subtotal = 0; $total_cal_y = 0;
