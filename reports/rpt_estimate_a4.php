...
if(isset($_REQUEST['view_estimate_id'])) {
    ...
    $pdf->SetX(105);
    $pdf->Cell(64,4,'Estimate No.            : '.$estimate_number,0,1,'L',0);
    $pdf->Cell(0,2,'',0,1,'L',0);
    $pdf->SetX(105);
    $pdf->Cell(64,4,'Date                        : '.$estimate_date,0,1,'L',0);
    $pdf->Cell(0,2,'',0,1,'L',0); 
    $pdf->SetX(105);
    ...
    $index = 0;
    $total_unit = $total_subunit = $estimate_subtotal = 0; $total_cal_y = 0;
    ...
    if(!empty($estimate_subtotal)) {
        $subtotal = $obj->numberFormat($estimate_subtotal,2);
        $pdf->SetFont('Arial','B',8);
        ...
        $total_amount_ = $estimate_subtotal;
