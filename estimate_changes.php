<?php
        ...
        <form class="poppins pd-20 redirection_form" name="estimate_form" method="POST">
            ...
            <div class="row justify-content-center p-3">
                ...
                <div class="col-lg-2 col-md-4 col-6 py-2 px-lg-1">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <input type="date" name="estimate_date" class="form-control shadow-none" placeholder="" required="" value="<?php if(!empty($estimate_date)) { echo $estimate_date; } ?>" max="<?php if(!empty($current_date)) { echo $current_date; } ?>">
                ...
                <div class="col-lg-3 col-md-4 col-6 py-2 px-lg-1">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <select class="select2 select2-danger" name="party_id" data-dropdown-css-class="select2-danger" style="width: 100%!important;" onchange="Javascript:HideDetails('party');GetVehicles('estimate');getPartyState(this.value);">
                                <?php if(!empty($show_estimate_id)) {  ?>
                        ...
                        <a href="Javascript:ViewPartyDetails('party');" class="<?php if(empty($show_estimate_id)){?>d-none<?php }?> details_element" style="font-size: 12px;font-weight: bold;">Click to view details</a>
                    </div>    
                </div>
            ...
            <div class= "row p-3">
                ...
                <div class="col-lg-3 col-md-3  px-lg-1 py-2 tax_cover <?php if($gst_option !='1'){?>d-none<?php } ?>">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                ...
                <div class="col-lg-3 col-md-3  px-lg-1 py-2 <?php if($gst_option !='1'){?>d-none<?php } ?> tax_cover1" id="tax_option_div">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                ...
                <div class="col-lg-3 col-md-3  px-lg-1 py-2 <?php if($tax_type !='2'){ ?>d-none <?php } ?> tax_cover2">
                    <div class="form-group">
            ...
            <div class="row">    
                ...
                <div class="col-md-12 py-3 text-center">
                    <button class="btn btn-dark" type="button" onClick="Javascript:SaveModalContent(event, 'estimate_form', 'estimate_changes.php', 'estimate.php');"> Submit </button>
                </div>
            </div>
    ...
    if(isset($_REQUEST['edit_id'])) {
        $estimate_date = ""; $estimate_date_error = ""; $party_id = ""; $party_id_error = "";$product_id = array(); $quantity = array(); $total_qty = array();$rate = array();  $final_rate =array(); $product_amount =array(); $product_error = ""; $product_names = array(); $amount =array();$round_off = ""; $sub_total = 0; $total_amount = 0; $unit_id = "";$unit_ids = array(); $unit_id_error ="";$charges_id = array(); $charges_names = array();
        $charges_values = array(); $charges_type = array(); $charges_total = array();  $is_discount =""; $discount_name = ""; $valid_estimate = ""; $form_name = "estimate_form"; $edit_id = ""; $discount_value =""; $discounted_total = 0;   $store_ids = array(); $gst_option_error = ""; $tax_type = ""; $tax_type_error = "";$tax_option = ""; $tax_option_error = ""; $overall_tax ="";$final_rate =array();$cgst_value = 0; $sgst_value = 0; $igst_value = 0; $gst_option ="";  $estimate_error = '';$product_tax =array(); $charges_tax =array();$company_state = ""; $party_state = ""; $total_tax_value = 0;
        ...
        $estimate_date_error = $valid->common_validation($estimate_date, 'Entry Date', '1');
        ...
        if(!empty($vehicle_id_error)) {
            if(!empty($valid_estimate)) {
                $valid_estimate = $valid_estimate." ".$valid->error_display($form_name, 'vehicle_id', $vehicle_id_error, 'select');
            }
            else {
                $valid_estimate = $valid->error_display($form_name, 'vehicle_id', $vehicle_id_error, 'select');
            }
        }
        ...
        if(!empty($vehicle_details_error)) {
            if(!empty($valid_estimate)) {
                $valid_estimate = $valid_estimate." ".$valid->error_display($form_name, 'vehicle_details', $vehicle_details_error, 'text');
            }
            else {
                $valid_estimate = $valid->error_display($form_name, 'vehicle_details', $vehicle_details_error, 'text');
            }
        }
        ...
        if(!empty($gst_option_error)) {
            if(!empty($valid_estimate)) {
                $valid_estimate = $valid_estimate." ".$valid->error_display($form_name, 'gst_option', $gst_option_error, 'text');
            }
            else {
                $valid_estimate = $valid->error_display($form_name, 'gst_option', $gst_option_error, 'text');
            }
        }
        ...
        if(!empty($product_id)) {
            for($i=0;$i<count($product_id); $i++) {
                ...
                if(!empty($rate[$i])) {
                    ...
                    if(!empty($rate_error)) {
                        if(!empty($valid_estimate)) {
                            $valid_estimate = $valid_estimate." ".$valid->row_error_display($form_name, 'rate[]', $rate_error, 'text','product_row', ($i+1));
                        }
                        else {
                            $valid_estimate = $valid->row_error_display($form_name, 'rate[]', $rate_error, 'text','product_row', ($i+1));
                        }
                    }
                }
            }
                ...
                if(!empty($charges_id[$i])) {
                    ...
                    if(isset($charges_values[$i])) {
                        ...
                        if(!empty($charges_error)) {
                            if(!empty($estimate_error)) {
                                $estimate_error = $estimate_error."<br>".$charges_error;
                            }
                            else {
                                $estimate_error = $charges_error;
                            }
                        }
                        else {
                            if(strpos($charges_values[$i], '%') !== false) {
                    ...
                    if(empty($estimate_error)) {
                        ...
                        if($gst_option == '1')
                        {
                            ...
                            {
                                $estimate_error = "Select Charges tax";
                            }
        ...
        if(!empty($total_amount)) {	
            ...
            if(!empty($round_off_value_error)) {
                if(!empty($valid_estimate)) {
                    $valid_estimate = $valid_estimate." ".$valid->error_display($form_name, 'round_off_value', $round_off_value_error, 'text');
                }
                else {
                    $valid_estimate = $valid->error_display($form_name, 'round_off_value', $round_off_value_error, 'text');
                }
            } else {
                if($round_off == '2')
                {
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                if(empty($edit_id)) {
                    ...
                    if(preg_match("/^\d+$/", $estimate_insert_id)) {
                        ...
                        $estimate_balance =1;
                        $job_update = 1;
                        $result = array('number' => '1', 'msg' => 'Estimate Entry Successfully Created','redirection_page' =>'estimate.php');					
                    }
                    else {
                    ...
                    if(preg_match("/^\d+$/", $getUniqueID)) {
                        $action = "Estimate Entry Updated. Bill No. - ".$estimate_number;

                        $columns = array(); $values = array();						
                        $columns = array('creator_name','bill_company_id', 'bill_company_details', 'estimate_date','party_id', 'party_name_mobile_city', 'party_details', 'gst_option', 'tax_type', 'tax_option', 'overall_tax', 'charges_tax', 'product_tax', 'party_state','vehicle_id','vehicle_number','vehicle_details','job_card_id','job_card_number','store_entry_id','store_entry_number','store_id','store_name','product_id', 'product_name', 'hsn_code', 'quantity', 'unit_id' ,'unit_name' ,'rate','final_rate', 'product_amount','amount', 'sub_total', 'discount', 'discount_value', 'discounted_total',  'charges_name','charges',  'charges_value','charges_total','cgst_value', 'sgst_value', 'igst_value', 'total_tax_value','round_off', 'total_amount', 'round_off_type' , 'round_off_value', 'discount_name',);
                ...
                if($job_update == 1) {
                    ...
                    $columns = array('estimate_status','estimate_id');
                    $values =array("'E'","'".$estimate_id."'");
                    $action = "Updated Estimate. Bill No. - ".$estimate_number;
    ...
    if(isset($_POST['page_number'])) {
        ...
        $total_records_list = $obj->getSalesList($from_date, $to_date,$show_bill,$party_id, 'estimate');
                                ...
                                ?>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink1">
                                            <li>
                                                <a class="dropdown-item" style="cursor:pointer;"  target="_blank" href="reports/rpt_estimate_a4.php?view_estimate_id=<?php if(!empty($list['estimate_id'])) { echo $list['estimate_id']; } ?>"><i class="fa fa-print"></i>&ensp;Print</a>
                                            </li>
                                            <?php 
    ...
    if(isset($_REQUEST['delete_estimate_id'])) {
        ...
        $delete_estimate_id = trim($delete_estimate_id);
        $msg = "";
        if(!empty($delete_estimate_id)) {	
            $estimate_unique_id = ""; $voucher_unique_id = ""; $voucher_id = "";
            ...
            if(preg_match("/^\d+$/", $estimate_unique_id)) {
                $bill_number = "";
                $bill_number = $obj->getTableColumnValue($GLOBALS['estimate_table'], 'estimate_id', $delete_estimate_id, 'estimate_number');
                ...
                $columns = array(); $values = array();			
                $obj->RevoteJobCard('estimate',$delete_estimate_id);
            }
            else {
