			...
			if (check_login_session == 1) {
				...
				if (parseFloat(all_errors_check) == 1) {
					...
					if (parseInt(add) == 1) {
						// if (parseInt(balance) == 1) {
							...
							jQuery.ajax({
								url: post_url, success: function (result) {
									...
									if (jQuery('input[name="voucher_party_id"]').length > 0) {
										jQuery('input[name="voucher_party_id"]').val(party_id);
									}
						...
						// }
						// else {
						// 	jQuery('.payment_row_table').before('<span class="infos w-50 text-center mb-3 fw-bold" style="font-size: 15px;">Amount is Greater than Available Balance</span>');
						// }
					}
					else {
...
function ShowAdvance(engineer_id){
    var post_url = "payment_bill_changes.php?get_engineer_adavance="+engineer_id;
    jQuery.ajax({
        url: post_url, success: function (result) {
            result = result.trim();
            if(engineer_id != ""){
                if(result != "" && result != '0') {
                    if(jQuery('#advance').length > 0) {
                        jQuery('#advance').removeClass('d-none');
                        jQuery('#advance').html('Advance Amount: Rs.'+result);
                    }
                }
                else {
                    if(jQuery('#advance').length > 0) {
                        jQuery('#advance').addClass('d-none');
                        // jQuery('#advance').html('Advance Amount: Rs. 0.00');
                        jQuery('#advance').html('');
                    }
                }
            }else{
