function show_to_store(from_store_id) {
	...
	jQuery.ajax({
		url: post_url, success: function (check_login_session) {
			if (check_login_session == 1) {
				...
				var post_url = "material_transfer_table.php?from_store_id=" + from_store_id;
				jQuery.ajax({
					url: post_url, success: function (to_store_id) {
						if (jQuery('select[name="to_store_id"]').length > 0) {
							jQuery('select[name="to_store_id"]').html(to_store_id);
                            GetStoreProduct();
						}
					}
				});
...
function GetStoreProduct() {
	...
	if (jQuery('select[name="from_store_id"]').length > 0) {
		store_id = jQuery('select[name="from_store_id"]').val();
	}
    if(store_id == '') {
        return false;
    }
	...
	jQuery.ajax({
		url: post_url, success: function (check_login_session) {
			if (check_login_session == 1) {
				...
				var from_store_id = ""; var to_store_id = "";
				if (jQuery('select[name="from_store_id"]').is(":visible")) {
					
					if(jQuery('select[name="from_store_id"]').length > 0) {
						from_store_id = jQuery('select[name="from_store_id"]').val();
						from_store_id = jQuery.trim(from_store_id);
						if(typeof from_store_id == "undefined" || from_store_id == "" || from_store_id == 0) {
							all_errors_check = 0;
                            throwerrormsg('from_store_id', 'select', 'From store',form_name );                            
						}
					}
				}else{
					if(jQuery('input[name="from_store"]').length > 0) {
						from_store_id = jQuery('input[name="from_store"]').val();
						from_store_id = jQuery.trim(from_store_id);
						if(typeof from_store_id == "undefined" || from_store_id == "" || from_store_id == 0) {
							all_errors_check = 0;
						}
				...
				if (jQuery('select[name="to_store_id"]').is(":visible")) {
					if(jQuery('select[name="to_store_id"]').length > 0) {
						to_store_id = jQuery('select[name="to_store_id"]').val();
						to_store_id = jQuery.trim(to_store_id);
						if(typeof to_store_id == "undefined" || to_store_id == "" || to_store_id == 0) {
							all_errors_check = 0;
                            throwerrormsg('to_store_id', 'select', 'To store',form_name );                            
						}
					}
				}else{
					if(jQuery('input[name="to_store"]').length > 0) {
						to_store_id = jQuery('input[name="to_store"]').val();
						to_store_id = jQuery.trim(to_store_id);
						if(typeof to_store_id == "undefined" || to_store_id == "" || to_store_id == 0) {
							all_errors_check = 0;
						}
                ...
                if(parseFloat(all_errors_check) == 1) {
                    ...
                    if(parseFloat(add) == 1) {
                        ...
                        var post_url = "";
                        if(form_name == "stock_adjustment_form"){
                            post_url = "stock_adjustment_table.php?product_row_index="+product_count+"&selected_product_id="+selected_product_id+"&selected_unit_id="+selected_unit_id+"&selected_quantity="+selected_quantity+"&selected_stock_action="+selected_stock_action+"&selected_store_id="+store_id;
                        }else{
                            post_url = "material_transfer_table.php?product_row_index="+product_count+"&selected_product_id="+selected_product_id+"&selected_unit_id="+selected_unit_id+"&selected_quantity="+selected_quantity+"&selected_stock_action="+selected_stock_action+"&selected_store_id="+store_id;
                        }
        ...
        url: post_url, success: function (check_login_session) {
            if (check_login_session == 1) {
                ...
                if (id_name == 'product_row') {
                    ...
                    if(product_count == 0){
                        if (jQuery('select[name="from_store_id"]').length > 0) { 
                            jQuery('select[name="from_store_id"]').attr('disabled',false);
                        }                                                                                        
                        if (jQuery('select[name="to_store_id"]').length > 0) { 
                            jQuery('select[name="to_store_id"]').attr('disabled',false);
                        }               
                    }
                }
