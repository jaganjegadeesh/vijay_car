...
function FormSubmit(event, form_name, submit_page, redirection_page) {
	...
	jQuery('form[name="' + form_name + '"]').find('.row:first').before('<div class="alert alert-danger alert-dismissible fade show mb-5" role="alert"> <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> Processing </div>');
	...
	jQuery.ajax({
		...
		success: function (data) {
			...
			if (x.number == '1') {
				jQuery('form[name="' + form_name + '"]').find('.row:first').before('<div class="alert alert-success alert-dismissible fade show" role="alert"> <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> ' + x.msg + ' </div>');
			...
			if (x.number == '2') {
				jQuery('form[name="' + form_name + '"]').find('.row:first').before('<div class="alert alert-danger alert-dismissible fade show" role="alert"> <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> ' + x.msg + ' </div>');
				if (jQuery('.submit_button').length > 0) {
...
function SendModalContent(form_name, post_send_file, redirection_file) {
	jQuery.ajax({
		...
		success: function (data) {
			...
			if (x.number == '1') {
				jQuery('form[name="' + form_name + '"]').find('.row:first').before('<div class="alert alert-success"> <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> ' + x.msg + ' </div>');
				setTimeout(function () {
					if (jQuery('.redirection_form').length > 0) {
						if ('addcustom' in x && x.addcustom !== '') {

							CloseCustomeModel(x.addcustom);
	...
	jQuery.ajax({
		url: post_url, success: function (check_login_session) {
			if (check_login_session == 1) {
				...
				console.log('page_title - ' + page_title + ', delete_content_id - ' + delete_content_id);
					...
					jQuery.ajax({
						url: post_url, success: function (result) {
							...
							if (intRegex.test(result) == true) {
								if (page_title == "quotation" || page_title == "estimate" || page_title == "invoice") {
									jQuery('#DeleteModal .modal-body').append('<div class="alert alert-success">  Successfully Cancel the ' + page_title.replace("_", " ") + ' </div>');
								}
								else {
									jQuery('#DeleteModal .modal-body').append('<div class="alert alert-success">  Successfully Delete the ' + page_title.replace("_", " ") + ' </div>');
								}
								setTimeout(function () {
...
function showerrormsg(selected_name, type, value,form_name) {
	console.log(selected_name, type, value, form_name);
	...
	console.log("<br>", $formGroup.find('span.infos').length);
	if ($formGroup.find('span.infos').length === 0) {
		console.log('if going');
		$formGroup.append(`<span class="infos text-danger d-block mt-1"><i class="fa fa-exclamation-circle"></i> ${error_type} the ${value}</span>`);
	}else{
		console.log('else going');
	}
}
