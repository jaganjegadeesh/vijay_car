		...
		url: post_url, success: function (check_login_session) {
			if (check_login_session == 1) {
				if (jQuery('#excel_upload_details_table').find('tbody').find('tr:nth-child(' + row_index + ')').length > 0) {
					...
					var hsn_code = "";
					if (jQuery(excel_row).find('input[name="hsn_codes"]').length > 0) {
						hsn_code = jQuery(excel_row).find('input[name="hsn_codes"]').val();
					...
					if (jQuery(excel_row).find('input[name="unit_names"]').length > 0) {
						unit_name = jQuery(excel_row).find('input[name="unit_names"]').val();
						unit_name = clean_value(unit_name);
						unit_name = jQuery.trim(unit_name);
					}

					var product_rate = "";
					if (jQuery(excel_row).find('input[name="product_rates"]').length > 0) {
						product_rate = jQuery(excel_row).find('input[name="product_rates"]').val();
						product_rate = clean_value(product_rate);
						product_rate = jQuery.trim(product_rate);
					}
					var product_tax = "";
					if (jQuery(excel_row).find('input[name="product_taxs"]').length > 0) {
						product_tax = jQuery(excel_row).find('input[name="product_taxs"]').val();
					...
					var post_url = "product_upload.php?product_name=" + product_name + "&hsn_code=" + hsn_code + "&unit_name=" + unit_name + "&product_rate=" + product_rate + "&excel_upload_type=" + excel_upload_type + "&file_name=" + file_name+ "&product_tax="+product_tax;
					jQuery.ajax({
						url: post_url, success: function (result) {
							if (result == 1) {
